 ...
 */
#include <eosio/bocc_mongo_db_plugin/bocc_mongo_db_plugin.hpp>
...
void print_accounts() {
  for(map<string, bool>::iterator iter = accounts.begin(); iter != accounts.end(); iter++) {
...
namespace eosio {
  ...
  static appbase::abstract_plugin& _bocc_mongo_db_plugin = app().register_plugin<bocc_mongo_db_plugin>();
  ...
  class bocc_mongo_db_plugin_impl {
    public:
      bocc_mongo_db_plugin_impl();
      ~bocc_mongo_db_plugin_impl();
  ...
  const action_name bocc_mongo_db_plugin_impl::newaccount = chain::newaccount::get_name();
  const action_name bocc_mongo_db_plugin_impl::setabi = chain::setabi::get_name();
  const action_name bocc_mongo_db_plugin_impl::updateauth = chain::updateauth::get_name();
  const action_name bocc_mongo_db_plugin_impl::deleteauth = chain::deleteauth::get_name();
  const permission_name bocc_mongo_db_plugin_impl::owner = chain::config::owner_name;
  const permission_name bocc_mongo_db_plugin_impl::active = chain::config::active_name;

  const std::string bocc_mongo_db_plugin_impl::block_states_col = "block_states";
  const std::string bocc_mongo_db_plugin_impl::blocks_col = "blocks";
  const std::string bocc_mongo_db_plugin_impl::trans_col = "transactions";
  const std::string bocc_mongo_db_plugin_impl::trans_traces_col = "transaction_traces";
  const std::string bocc_mongo_db_plugin_impl::action_traces_col = "action_traces";
  const std::string bocc_mongo_db_plugin_impl::accounts_col = "accounts";
  const std::string bocc_mongo_db_plugin_impl::pub_keys_col = "pub_keys";
  const std::string bocc_mongo_db_plugin_impl::account_controls_col = "account_controls";

  bool bocc_mongo_db_plugin_impl::filter_include( const account_name& receiver, const action_name& act_name,
      const vector<chain::permission_level>& authorization ) const
  {
    bool include = false;
    if( filter_on_star ) {
  ...
  bool bocc_mongo_db_plugin_impl::filter_include( const transaction& trx ) const
  {
  ...
  template<typename Queue, typename Entry>
    void bocc_mongo_db_plugin_impl::queue( Queue& queue, const Entry& e ) {
  ...
  void bocc_mongo_db_plugin_impl::accepted_transaction( const chain::transaction_metadata_ptr& t ) {
    try {
  ...
  void bocc_mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
    try {
      ...
      // users should not be using the bocc_mongo_db_plugin in that mode anyway.
      //
      // Allow logging traces if node is a producer for testing purposes, so a single nodeos can do both for testing.
      //
      // It is recommended to run bocc_mongo_db_plugin in read-mode = read-only.
      //
  ...
  void bocc_mongo_db_plugin_impl::applied_irreversible_block( const chain::block_state_ptr& bs ) {
  }

  void bocc_mongo_db_plugin_impl::accepted_block( const chain::block_state_ptr& bs ) {
    if (bs->block_num % 4 == 0) {
      future<cpp_redis::reply> result =  client.lrange("eos:accounts:wallet:add", 0,  -1);
      client.sync_commit();
      auto adds = result.get().as_array();

      for (int index = 0; index != adds.size(); index++){
        string value = (adds[index]).as_string();
        accounts.insert(pair<string,bool>(value, true));
      }
      client.ltrim("eos:accounts:wallet:add", adds.size(), -1);
      client.sync_commit();
    }
  ...
  void bocc_mongo_db_plugin_impl::consume_blocks() {
    try {
      ...
      ilog("bocc_mongo_db_plugin consume thread shutdown gracefully");
    } catch (fc::exception& e) {
  ...
  void bocc_mongo_db_plugin_impl::purge_abi_cache() {
  ...
  optional<abi_serializer> bocc_mongo_db_plugin_impl::get_abi_serializer( account_name n ) {
    using bsoncxx::builder::basic::kvp;
    using bsoncxx::builder::basic::make_document;
    if( n.good()) {
      try {
  ...
  template<typename T>
    fc::variant bocc_mongo_db_plugin_impl::to_variant_with_abi( const T& obj ) {
  ...
  void bocc_mongo_db_plugin_impl::process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
    try {
      if( start_block_reached ) {
  ...
  void bocc_mongo_db_plugin_impl::process_applied_transaction( const chain::transaction_trace_ptr& t ) {
    try {
  ...
  void bocc_mongo_db_plugin_impl::process_irreversible_block(const chain::block_state_ptr& bs) {
    try {
      if( start_block_reached ) {
  ...
  void bocc_mongo_db_plugin_impl::process_accepted_block( const chain::block_state_ptr& bs ) {
    try {
      if( start_block_reached ) {
  ...
  void bocc_mongo_db_plugin_impl::_process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
  ...
  bool
    bocc_mongo_db_plugin_impl::add_action_trace( mongocxx::bulk_write& bulk_action_traces, const chain::action_trace& atrace,
        const chain::transaction_trace_ptr& t,
        bool executed, const std::chrono::milliseconds& now )
    {
      using namespace bsoncxx::types;
      ...
      if( start_block_reached && store_action_traces &&
        ...
        string action_name = "";
        string trx_id = base.trx_id;
        string contract_name = "";
        string from = "";
        string to = "";
        string quantity = "";
        string memo = "";
        string block_num = to_string(base.block_num);
        string block_time = "";
        string trx_status = string(t->receipt->status);
        string issuer = "";
        ...
        try {
          ...
          auto doc = action_traces_doc.view();
          if (base.act.name == "transfer") {
            from = (((doc["act"]["data"]["from"]).get_utf8()).value).to_string();
            to = (((doc["act"]["data"]["to"]).get_utf8()).value).to_string();
            quantity = (((doc["act"]["data"]["quantity"]).get_utf8()).value).to_string();
            memo = (((doc["act"]["data"]["memo"]).get_utf8()).value).to_string();
            block_time = (((doc["block_time"]).get_utf8()).value).to_string();
            contract_name = (((doc["act"]["account"]).get_utf8()).value).to_string();
            action_name = (((doc["act"]["name"]).get_utf8()).value).to_string();
            receiver = (((doc["receipt"]["receiver"]).get_utf8()).value).to_string();
            try {
              global_sequence = std::to_string(((doc["receipt"]["global_sequence"]).get_int32()).value);
            } catch (std::exception& e) {
              global_sequence = std::to_string(((doc["receipt"]["global_sequence"]).get_int64()).value);
            }
          }

          if (base.act.name == "create") {
            block_time = (((doc["block_time"]).get_utf8()).value).to_string();
            action_name = (((doc["act"]["name"]).get_utf8()).value).to_string();
            contract_name = (((doc["act"]["account"]).get_utf8()).value).to_string();
            issuer = (((doc["act"]["data"]["issuer"]).get_utf8()).value).to_string();
            maximum_supply = (((doc["act"]["data"]["maximum_supply"]).get_utf8()).value).to_string();
          }
        } catch( bsoncxx::exception& ) {
          try {
        ...
        if (base.act.name == "transfer" && contract_name != receiver) {
          return added;
        }
        if( t->receipt.valid() ) {
        ...
        if (base.act.name == "transfer") {
          map<string,bool >::iterator it = accounts.find(from);
          if (it != accounts.end()) {
            added = true;
          }

          if (!added) {
            map<string,bool >::iterator it = accounts.find(to);
            if (it != accounts.end()) {
              added = true;
            }
          }
  ...
  void bocc_mongo_db_plugin_impl::_process_applied_transaction( const chain::transaction_trace_ptr& t ) {
    using namespace bsoncxx::types;
    using bsoncxx::builder::basic::kvp;
  ...
  void bocc_mongo_db_plugin_impl::_process_accepted_block( const chain::block_state_ptr& bs ) {
    using namespace bsoncxx::types;
  ...
  void bocc_mongo_db_plugin_impl::_process_irreversible_block(const chain::block_state_ptr& bs)
  {
    using namespace bsoncxx::types;
    using namespace bsoncxx::builder;
    using bsoncxx::builder::basic::make_document;
  ...
  void bocc_mongo_db_plugin_impl::add_pub_keys( const vector<chain::key_weight>& keys, const account_name& name,
      const permission_name& permission, const std::chrono::milliseconds& now )
  {
    using bsoncxx::builder::basic::kvp;
    using bsoncxx::builder::basic::make_document;
  ...
  void bocc_mongo_db_plugin_impl::remove_pub_keys( const account_name& name, const permission_name& permission )
  {
    using bsoncxx::builder::basic::kvp;
    using bsoncxx::builder::basic::make_document;
  ...
  void bocc_mongo_db_plugin_impl::add_account_control( const vector<chain::permission_level_weight>& controlling_accounts,
      const account_name& name, const permission_name& permission,
      const std::chrono::milliseconds& now )
  {
    using bsoncxx::builder::basic::kvp;
  ...
  void bocc_mongo_db_plugin_impl::remove_account_control( const account_name& name, const permission_name& permission )
  {
    using bsoncxx::builder::basic::kvp;
    using bsoncxx::builder::basic::make_document;
  ...
  namespace {
    void create_account( mongocxx::collection& accounts, const name& name, std::chrono::milliseconds& now ) {
      ...
      vector<string> values {name_str};
      client.sadd("eos:accounts:all", values);
      client.sync_commit();
  ...
  void bocc_mongo_db_plugin_impl::update_account(const chain::action& act)
  {
    using bsoncxx::builder::basic::kvp;
    using bsoncxx::builder::basic::make_document;
    using namespace bsoncxx::types;
  ...
  bocc_mongo_db_plugin_impl::bocc_mongo_db_plugin_impl()
  {
  }
  ...
  bocc_mongo_db_plugin_impl::~bocc_mongo_db_plugin_impl() {
    if (!startup) {
      try {
        ilog( "bocc_mongo_db_plugin shutdown in process please be patient this can take a few minutes" );
        done = true;
        condition.notify_one();
        ...
        elog( "Exception on bocc_mongo_db_plugin shutdown of consume thread: ${e}", ("e", e.what()));
      }
    }
  }

  void bocc_mongo_db_plugin_impl::wipe_database() {
  ...
  void bocc_mongo_db_plugin_impl::init() {
    using namespace bsoncxx::types;
    using bsoncxx::builder::basic::make_document;
    using bsoncxx::builder::basic::kvp;
  ...
  bocc_mongo_db_plugin::bocc_mongo_db_plugin()
    :my(new bocc_mongo_db_plugin_impl)
  {
  }
  ...
  void bocc_mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
  {
    cfg.add_options()
      ...
      ("redis-ip-address", bpo::value<std::string>()->default_value("127.0.0.1"), "127.0.0.1")
      ("redis-port", bpo::value<std::uint32_t>()->default_value(6379), "6379")
      ;
  }

  void bocc_mongo_db_plugin::plugin_initialize(const variables_map& options)
  {
    try {
      if( options.count( "mongodb-uri" )) {
        ilog( "initializing bocc_mongo_db_plugin" );
        my->configured = true;
        ...
        wlog( "eosio::bocc_mongo_db_plugin configured, but no --mongodb-uri specified." );
        wlog( "bocc_mongo_db_plugin disabled." );
      }
      ...
      client.smembers("eos:accounts:wallet:all", [](cpp_redis::reply &results) {
        std::vector<cpp_redis::reply> members = results.as_array();
        for (auto member = members.begin(); member != members.end(); member++){
          accounts.insert(pair<string,bool>(member->as_string(), true));
        }
      });
      client.sync_commit();
      print_accounts();
  ...
  void bocc_mongo_db_plugin::plugin_startup()
  {
  }

  void bocc_mongo_db_plugin::plugin_shutdown()
  {
